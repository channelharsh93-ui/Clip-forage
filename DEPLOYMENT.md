...
- Import the repository root. The included root `vercel.json` builds `frontend` and publishes `frontend/dist`.
- Set Vercel **Root Directory** to `frontend`. The included `frontend/vercel.json` handles SPA routes.
